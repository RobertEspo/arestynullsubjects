...
mono = ggplot(data=L2H, aes(x=mono, y=besasp_total, color=heritage_, shape=heritage_)) +
  ...
  geom_point(size=5) +
  labs(title="L2 Caretakers' Perceptions (mono) vs Child's BESA score", x="Parent's perception against monolinguals", y="Child's BESA score", shape="heritage?") +  
...
bi = ggplot(data=L2H, aes(x=bi, y=besasp_total, color=heritage_, shape=heritage_)) +
  geom_smooth(method="lm", se=FALSE) +
  geom_point(size=5) +
  labs(title="L2 Caretakers' Perceptions (bi) vs Child's BESA score", x="Parent's perception against bilinguals", y="Child's BESA score", shape="heritage?") +  
  scale_y_continuous(breaks = round(seq(min(0), max(10), by = 1),1)) +
  xlim(0,5) +
  scale_color_manual(name=NULL,
                     labels = c("L2", "Heritage"),
